...
function displayTotalLikes(){
  ...
  <span class="visuallyhidden"> Le nombre de likes de ce photographe est de</span>${nbrTotalLikes}<em class="fa-solid fa-heart" aria-hidden="true"></em>`
}

function displayMediumLike(id, likes, liked, title){
  const like = document.querySelector(`#like_${id}`);
  like.innerHTML = `${likes}<em class="fa-solid fa-heart" aria-hidden="true"></em>`;
  if(liked){
